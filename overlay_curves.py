...
import os
import utils.array_util as array_util
...
def calculate_information_for_curves(gts, preds):
    ...
    # print(gts.shape , np.squeeze(preds).shape)
    fscore = (2 * prec * rec) / (prec + rec)
    print(fscore)
    # f1 = sklearn.metrics.f1_score(gts , np.squeeze(preds))
    # print("f1score=" , f1)
    # print(prec , rec)
    ap = sklearn.metrics.average_precision_score(gts, preds)
    
    return fpr, tpr, auc, prec, rec, ap, fscore; 
    

ground_truth = pd.read_csv("./temporal-annotations.txt", header=None, index_col=0)
...
for idx, row in ground_truth.iterrows():
    c3d_preds_file_path = os.path.join(cfg.preds_path, idx)
...
(
    ...
    prec_c3d, rec_c3d, ap_c3d ,fscore
...
plt.plot([i for i in range(len(fscore))  ] , fscore );
plt.ylim(0,0.5)

plt.show()
print("F1 Mesaure = " , fscore)