...
def visualize_predictions1(video_path, predictions, save_path):
    frames = get_video_frames(video_path)
    assert len(frames) == len(predictions)
    
    plt.plot(range(0, len(frames))  , predictions , '-' );
    plt.show()
def visualize_predictions(video_path, predictions, save_path):
    ...
    if save_path:
        FFwriter = mp.animation.FFMpegWriter()
        anim.save(save_path,dpi =200 , writer = FFwriter)
