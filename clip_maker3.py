...
INPUT_VIDEO_DIR = "C:/Users/Asus/Work/anamoly detect/Oct2022/AnomalyDetection_CVPR18/GUI/public/Files";
OUTPUT_DIR_VIDEO= "C:/Users/Asus/Work/anamoly detect/Oct2022/AnomalyDetection_CVPR18/GUI/public/Inter";
...
def break_video_into_clips(input_folder_path ,video_name , output_folder_path , basename):
    start = 0;
    duration = 30;
    end = get_duration_video(os.path.join(input_folder_path , video_name));
    folder_path =  os.path.join(output_folder_path , video_name.split(".")[0]); 
...
def delete_if_Exists(outputPath , video):
    abs_path = os.path.join(outputPath , video.split(".")[0]);
    print(abs_path);
    if(os.path.exists(abs_path)):
        for fl in os.listdir(abs_path):
            
            os.remove(os.path.join(abs_path , fl));
        os.rmdir(abs_path);

video_n =sys.argv[1];
print(video_n);

print("preapring video {}".format(video_n));

delete_if_Exists(OUTPUT_DIR_VIDEO ,  video_n);

    
break_video_into_clips( INPUT_VIDEO_DIR, video_n , OUTPUT_DIR_VIDEO , OUTPUT_VIDEO_BASE_NAMES);
